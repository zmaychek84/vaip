...
/*
    { "name": "vaip_pass_dd_merge_qconv2matmul_5",
       "plugin": "vaip-pass_dd_merge_qconv2matmul_5",
       "disabled": false
    }
*/
namespace {
using namespace vaip_core;
struct Dd_merge_qconv2matmul_5 {
  Dd_merge_qconv2matmul_5(IPass& self) : self_{self} {}
  std::unique_ptr<Rule> create_rule(IPass* self) {
    auto conv2matmul = vaip::pattern_zoo::get_pattern("m_qconv2matmul_5");
    ...
    return Rule::create_rule(
        conv2matmul, [=](onnxruntime::Graph* graph, binder_t& binder) -> bool {
          ...
          auto out_scale_node = binder["constant_16"];
          auto out_zp_node = binder["constant_17"];
          auto bias_node = binder["constant_13"];
          auto bias_scale_node = binder["constant_14"];
          auto bias_zp_node = binder["constant_15"];
          NodeArg* no_op_node_arg = nullptr;

          bool no_op_in_parent = qconv2matmul::check_no_op_child(
              *graph, out_node.node, no_op_node_arg, "Squeeze");
          if (no_op_in_parent) {
          ...
          if (weight_data_type == 2) {
            ...
            std::vector<std::string> input_types{"uint16", "int4",  "int64",
                                                 "int32",  "int32", "int32"};
            std::vector<std::string> output_types{"uint16"};
            gsl::span<const uint16_t> bias;
            bias = node_arg_get_const_data_as_u16s(*graph, *bias_node.node_arg);
            auto bias_scale = node_arg_get_const_data_as_float(
                *graph, *bias_scale_node.node_arg);
            auto bias_zero_point =
                node_arg_get_const_data_as_u16(*graph, *bias_zp_node.node_arg);

            auto [C0, C1, C2, conv_shift, shft_c2] = vaip::dd::qmatmulcalc::
                dq_uint16A_int4W_conv_chwise_bias_add_q_param_gen(
                    in_scale, in_zero_point, wts_vec, weights_scale,
                    wts_zp_vec_orig, wts_shape, bias, bias_scale,
                    bias_zero_point,
                    /*bias_zero_point*/ out_scale, out_zero_point);
...
DEFINE_VAIP_PASS(Dd_merge_qconv2matmul_5, vaip_pass_dd_merge_qconv2matmul_5)
