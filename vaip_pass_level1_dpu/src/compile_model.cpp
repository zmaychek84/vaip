...
create_compile_attrs(const PassContext& context,
  ...
  if (context.get_provider_option("enable_preemption").has_value()) {
    std::string str = context.get_provider_option("enable_preemption").value();
    bool enabled = (str == "1");
    compile_options->set_attr<bool>("enable_preemption", enabled);
  }
  if (context.get_provider_option("preemption_lib_path").has_value()) {
    std::string str =
        context.get_provider_option("preemption_lib_path").value();
    compile_options->set_attr<std::string>("preemption_lib_path", str);
  }
