...
DEF_ENV_PARAM(DEBUG_DD_MERGE_DQOP_ONNX, "0")
#define MY_LOG(n) LOG_IF(INFO, ENV_PARAM(DEBUG_DD_MERGE_DQOP_ONNX) >= n)
...
struct Dd_merge_dqop_onnx {
  Dd_merge_dqop_onnx(IPass& self) : self_{self} {}
  std::unique_ptr<Rule> create_rule(IPass* self) {
    auto dq = vaip::pattern_zoo::get_pattern("m_dq_onnx");
    ...
    return Rule::create_rule(
        dq, [=](onnxruntime::Graph* graph, binder_t& binder) -> bool {
          ...
          auto [meta_def, err] =
              self->try_fuse(*graph, "dqop_" + node_name, inputs, outputs,
...
DEFINE_VAIP_PASS(Dd_merge_dqop_onnx, vaip_pass_dd_merge_dqop_onnx)
