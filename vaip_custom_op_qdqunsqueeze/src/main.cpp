...
static vaip_core::ExecutionProvider* create_execution_provider_imp(
  ...
  return new vaip_core::ExecutionProviderImp<
      vaip_qdqunsqueeze_custom_op::MyCustomOp>(context, meta_def);
}

#if VAIP_CUSTOM_OP_QDQUNSQUEEZE_USE_DLL == 0
#  include <vitis/ai/plugin.hpp>
namespace {
static vitis::ai::StaticPluginRegister
    __register("vaip_custom_op_QDQUNSQUEEZE", "create_execution_provider",
               (void*)&create_execution_provider_imp);
} // namespace
extern "C" {
void* vaip_custom_op_qdqunsqueeze__hook = &__register;
}
#else
...
extern "C" {
void* vaip_custom_op_qdqunsqueeze__hook = nullptr;
}
#endif
