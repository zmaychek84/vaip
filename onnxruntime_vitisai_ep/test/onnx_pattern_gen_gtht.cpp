...
public:
  ...
  const std::string& node_arg_name() const {
    ...
    return VAIP_ORT_API(node_arg_get_name_unsafe)(*node_input_.node_arg);
    ;
  }
  /**
  ...
  mmd_build_self(const std::vector<std::string>& outputs) const {
    ...
    if (hint_ == "constant") {
      ...
      auto node_arg_name =
          VAIP_ORT_API(node_arg_get_name_unsafe)(*node_input_.node_arg);
      // node_arg_get_name(*node_input_.node_arg);
  ...
  std::string node_label(const std::string format, const std::string& cxx_name,
    ...
    auto node_arg_name = VAIP_ORT_API(node_arg_get_name_unsafe)(*ni.node_arg);
    // node_arg_get_name(*ni.node_arg);
    auto op_type =
...
public:
  NodePatternBuiler(const std::vector<const NodeArg*>& graph_inputs)
  ...
  std::shared_ptr<NodePattern> build_node(const Node& node) {
    ...
    for (auto ni : node_inputs) {
      std::string ni_name =
          VAIP_ORT_API(node_arg_get_name_unsafe)(*ni.node_arg);
      bool is_graph_input =
          std::any_of(graph_inputs_.begin(), graph_inputs_.end(),
                      [&ni_name](const NodeArg* node_arg) -> bool {
                        return node_arg_get_name(*node_arg) == ni_name;
                      });
      bool is_lstm_opt_input = false;
      if (ni_name == "") {
        ni_name = VAIP_ORT_API(node_get_name)(node) + "/opt_sequence_lens";
        is_lstm_opt_input = true;
      }
      ...
      if (it != map_node_patterns_.end()) {
        ...
        if (ni.node == nullptr) {
          if (is_lstm_opt_input || is_graph_input) {
            arg_node_pattern = build_input_pattern(*ni.node_arg);
          } else {
            arg_node_pattern = build_constant_pattern(*ni.node_arg);
          }
  ...
  std::shared_ptr<NodePattern> push_back_build_node_pattern(
    ...
    std::string node_arg_name =
        VAIP_ORT_API(node_arg_get_name_unsafe)(*node_input.node_arg);
...
std::vector<vaip_cxx::NodeConstRef> get_nodes(vaip_cxx::GraphConstRef graph,
  ...
  // force topological order.
  std::vector<vaip_cxx::NodeConstRef> topo = graph.nodes_in_topological_order();
  std::sort(nodes.begin(), nodes.end(), [&topo](const auto& a, const auto& b) {
    size_t topo_idx_a = std::find(topo.begin(), topo.end(), a) - topo.begin();
    size_t topo_idx_b = std::find(topo.begin(), topo.end(), b) - topo.begin();
...
int main(int argc, char* argv[]) {
  try {
    ...
    std::cout << "- Generate pattern "
                 "-----------------------------------------"
              << std::endl;
    auto inputs = *VAIP_ORT_API(graph_get_inputs_unsafe)(graph);
    auto builder = NodePatternBuiler(inputs);
