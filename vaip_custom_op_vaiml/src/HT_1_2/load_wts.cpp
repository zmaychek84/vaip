...
int htGenerateLstmInput(const LstmSettings& s, struct lstm_init_wts& lstm_in,
  ...
  memcpy(result, wts_v.data(), wts_v.size() - 64);
  if (s.layer_id == 320) {
  ...
  //  WTensor<double> SWk_dbl =
  //  WTensor<double>::createFromVector(SWk_i32.data()); WTensor<double> SRk_dbl
  //  = WTensor<double>::createFromVector(SRk_i32.data());
  ...
  //      w0_u8.reshape({4, 1024, s.len_x}).reorder(order).reshape({4096,
  //      s.len_x});
  //  WTensor<uint8_t> r1_u8 =
  //      r0_u8.reshape({4, 1024, s.len_h}).reorder(order).reshape({4096,
  //      s.len_h});
  ...
  //  }
  //  printf("####: %s: wrote %d bytes of data\n", s.layer_name.c_str(), p -
  //  wts); size_t wts_size = p - wts; saveMemoryToCache((const char*)wts,
  //  wts_size, context, s.layer_name); return 0;
}

std::vector<uint8_t>
ht_wts_gen_lstm_b2b(lstm_init_wts& param,
                    const vaip_core::PassContext& context) {
...
wts_gen_matmul(int8_t* mat_B, uint32_t M, uint32_t K, uint32_t N, uint32_t sv_M,
  ...
  mm_add_rtp[0] = sv_K;
  mm_add_rtp[1] = sv_N;
  mm_add_rtp[2] = shft_c2;
  mm_add_rtp[3] = K / sv_K;
  mm_add_rtp[15] = 2; // sk op code
  return res;
}
void rtp_gen_add(double ifm1_scale, int64_t ifm1_zero_point, double ifm2_scale,
                 int64_t ifm2_zero_point, double ofm_scale,
                 int64_t ofm_zero_point, float ifm_scale_refactor,
                 float ofm_scale_refactor, uint32_t* mm_add_rtp) {
#define ht_max(a, b) ((a < b) ? b : a)
  ifm1_zero_point = (ifm1_zero_point + 128) / ifm_scale_refactor;
  ifm2_zero_point = ifm2_zero_point + 128;
  ofm_zero_point = (ofm_zero_point + 128) / ofm_scale_refactor;
  ifm1_scale = ifm1_scale * ifm_scale_refactor;
  ofm_scale = ofm_scale * ofm_scale_refactor;

  int ifm1_shift = std::floor(-std::log2(ifm1_scale / ofm_scale) + 30);
  int ifm2_shift = std::floor(-std::log2(ifm2_scale / ofm_scale) + 30);

  double signed_zp = ofm_zero_point - ifm1_scale * ifm1_zero_point / ofm_scale -
                     ifm2_scale * ifm2_zero_point / ofm_scale;
  int zero_point_shift = std::floor(-std::log2(std::abs(signed_zp)) + 30);
  int ofm_shift = ht_max(ht_max(ifm1_shift, ifm2_shift), zero_point_shift);
  ...
  ofm_shift = static_cast<int>(ofm_shift);
  ifm1_shift = static_cast<int>(ofm_shift - ifm1_shift);
  ifm2_shift = static_cast<int>(ofm_shift - ifm2_shift);
  zero_point_shift = static_cast<int>(ofm_shift - zero_point_shift);

  mm_add_rtp[0] = 0x00000040;
  mm_add_rtp[1] = ifm1_coeff;
  mm_add_rtp[2] = ifm2_coeff;
  mm_add_rtp[3] = zero_point_coeff;
  mm_add_rtp[4] = (zero_point_shift << 24) | (ifm2_shift << 16) |
                  (ifm1_shift << 8) | ofm_shift;
  mm_add_rtp[15] = 3; // sk op code
#undef ht_max
}
} // namespace vaip_vaiml_custom_op
