...
static vaip_core::ExecutionProvider* create_execution_provider_imp(
  ...
  return new vaip_core::ExecutionProviderImp<
      vaip_gemm_dynamic_custom_op::MyCustomOp>(context, meta_def);
}

#if VAIP_CUSTOM_OP_GEMM_DYNAMIC_USE_DLL == 0
#  include <vitis/ai/plugin.hpp>
namespace {
static vitis::ai::StaticPluginRegister
    __register("vaip_custom_op_GEMM_DYNAMIC", "create_execution_provider",
               (void*)&create_execution_provider_imp);
} // namespace
extern "C" {
void* vaip_custom_op_gemm_dynamic__hook = &__register;
}
#else
...
extern "C" {
void* vaip_custom_op_gemm_dynamic__hook = nullptr;
}
#endif
