...
template <typename T>
static void writeToFile(std::string filename, T* data, size_t size) {
...
DEF_ENV_PARAM(DEBUG_VAIP_GQA_CUSTOM_OP, "0")
...
#define MY_LOG(n) LOG_IF(INFO, ENV_PARAM(DEBUG_VAIP_GQA_CUSTOM_OP) >= n)

namespace vaip_gqa_custom_op {
std::once_flag MyCustomOp::initFlag;
...
void MyCustomOp::get_rope_cache(const std::shared_ptr<MetaDefProto>& meta_def) {
  std::string strsp = meta_def->generic_param().at("cos_cache_shape");
  auto pos1 = strsp.find_first_of("[");
  auto pos2 = strsp.find_first_of(",");
  auto pos3 = strsp.find_first_of("]");
  auto shape_cs_0 = stoi(strsp.substr(pos1 + 1, pos2 - pos1 - 1));
  auto shape_cs_1 = stoi(strsp.substr(pos2 + 1, pos3 - pos2 - 1));
  headsize_ = 2 * shape_cs_1;
  std::cout << "cos shape:(" << shape_cs_0 << "," << shape_cs_1 << ")";

  std::string inputbin_cos = meta_def->generic_param().at("cos_cache_file");
  std::string inputbin_sin = meta_def->generic_param().at("sin_cache_file");

  unsigned int size_cos = (unsigned int)fs::file_size(inputbin_cos);
  std::vector<float> cos_file;
  cos_file.reserve(size_cos / sizeof(float));
  auto icos = std::ifstream(inputbin_cos, std::ios::in | std::ios::binary);
  icos.read((char*)cos_file.data(), size_cos);
  const float* cos_data = cos_file.data();

  unsigned int size_sin = (unsigned int)fs::file_size(inputbin_sin);
  std::vector<float> sin_file;
  sin_file.reserve(size_sin / sizeof(float));
  auto isin = std::ifstream(inputbin_sin, std::ios::in | std::ios::binary);
  isin.read((char*)sin_file.data(), size_sin);
  const float* sin_data = sin_file.data();
  ...
  cs_1 = shape_cs_1;
  std::cout << "cs_1:" << cs_1 << std::endl;

  // malloc last-dim dup-ed cos-sin bf16 buf
#ifdef _WIN32
  ...
  // convert last-dim dup-ed float cos/sin to bf16
...
void MyCustomOp::transpose0213(uint16_t* output_data, uint16_t* input_data,
...
///
///
void pad_concat_kv(uint16_t* dst, float* past, uint16_t* current, int N_kv,
  ...
  for (int n = 0; n < N_kv; n++) {
    float_to_bfloat16_avx512_unrolled(past + n * past_s_stride * H,
                                      dst + n * T_pad * H, past_S * H);
...
void save_present_kv(dtype* dst, dtype* past, uint16_t* current, int N_kv,
                     int past_S, int H, int T_buffer, int chunk_sz = 1) {
  for (int n = 0; n < N_kv; n++) {
    bfloat16_to_float_avx512_unrolled(current + n * chunk_sz * H,
                                      dst + n * T_buffer * H + past_S * H,
                                      chunk_sz * H);
  }
}

void MyCustomOp::set_params() {
...
MyCustomOp::MyCustomOp(std::shared_ptr<const PassContext> context,
  ...
  do_rotary_ = stoi(meta_def->generic_param().at("do_rotary"));
  kv_num_heads_ = stoi(meta_def->generic_param().at("kv_num_heads"));
  num_heads_ = stoi(meta_def->generic_param().at("num_heads"));
  if (instances__ == 0) {
    is_packed_qkv_ = stoi(meta_def->generic_param().at("is_packed_qkv"));
    std::cout << "is_packed_qkv_:" << is_packed_qkv_ << std::endl;
    get_rope_cache(meta_def);
    ...
    if (ENV_PARAM(USE_AIE_GQA) == 1) {
      ...
      MY_LOG(2) << "initialization for gqa aie custom-op done." << std::endl;
    }
  }
  instances__++;
}
...
void MyCustomOp::aie_execute_rope_mha(
    ...
    int past_k_size, const int T, std::shared_future<void> cache_v,
  ...
  pad_concat_kv(bmm1_inputs[1].map<uint16_t*>(), past_k_data.cast<float>(),
  ...
  // auto softmax_mask = softmax_->get_inputs()[1];
...
bool isBf16Model(const DataPtrWrapper& packed_qkv, DataPtrWrapper& out,
  ...
  return packed_qkv.is_bf16() && out.is_bf16();
}
...
void MyCustomOp::Compute(const OrtApi* api, OrtKernelContext* context) const {
  ...
  Ort::ConstValue past_k;
  Ort::ConstValue past_v;
  Ort::ConstValue seqlens_k;
  Ort::ConstValue total_seqlen;
  Ort::ConstValue cos_cache;
  Ort::ConstValue sin_cache;
  Ort::ConstValue packed_qkv = ctx.GetInput(0);
  if (is_packed_qkv_) {
    past_k = ctx.GetInput(1);
    past_v = ctx.GetInput(2);
    seqlens_k = ctx.GetInput(3);
    total_seqlen = ctx.GetInput(4);
    // cos_cache = ctx.GetInput(5);
    // sin_cache = ctx.GetInput(6);
  } else {
    key = ctx.GetInput(1);
    value = ctx.GetInput(2);
    past_k = ctx.GetInput(3);
    past_v = ctx.GetInput(4);
    seqlens_k = ctx.GetInput(5);
    total_seqlen = ctx.GetInput(6);
    cos_cache = ctx.GetInput(7);
    sin_cache = ctx.GetInput(8);
  }
  ...
  Ort::UnownedValue present_k = ctx.GetOutput(0, present_k_shape);
  Ort::UnownedValue present_v = ctx.GetOutput(1, present_v_shape);

  bool bo_sharing = false;
  __TIC__(Compute)
  ...
  int S = seq_len_k[0] + 1;
  const int past_seqlen = S - seq_len;
  ...
  if (!bo_sharing) {
    auto output = ctx.GetOutput(2, output_shape);
    ...
    if (isBf16Model(qkv_data, output_data, present_k_data, present_v_data)) {
      ...
      auto func_pad_concat_v = [&]() {
        ...
        pad_concat_kv(bmm2_inputs[1].map<uint16_t*>(),
                      past_v_data.cast<float>(), bf16_V_BNSH, N_kv, past_seqlen,
                      S_pad, H, true, T_buffer, seq_len);
      ...
      std::function<void(uint16_t*)> func_save_present_k =
          [&](uint16_t* k_ptr) {
            ...
            save_present_kv<float>(present_k_data.cast<float>(),
                                   past_k_data.cast<float>(), k_ptr, N_kv,
                                   past_seqlen, H, T, seq_len);
            __TOC__(SavePresentK)
          };
      std::function<void()> func_save_present_v = [&]() {
        __TIC__(SavePresentV)
        save_present_kv<float>(present_v_data.cast<float>(),
                               past_v_data.cast<float>(), bf16_V_BNSH, N_kv,
                               past_seqlen, H, T, seq_len);
      ...
      if (ENV_PARAM(USE_AIE_RoPE) == 1 &&
        ...
        __TOC__(PadQK)
        if (!bo_sharing) {
          const_cast<MyCustomOp*>(this)->aie_execute_rope_mha(
              output_data.cast<uint16_t>(), N_q, N_kv, seq_len, seq_len_pad, S,
              S_pad, H, past_present_share_buffer, T, past_k_data, past_k_size,
              T, rst_pad_concat_v, func_save_present_k);
        } else {
          const_cast<MyCustomOp*>(this)->aie_execute_rope_mha(
              nullptr, N_q, N_kv, seq_len, seq_len_pad, S, S_pad, H,
              past_present_share_buffer, T, past_k_data, past_k_size, T,
