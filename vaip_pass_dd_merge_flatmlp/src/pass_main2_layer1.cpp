...
struct MergeFlatMLP3 {
  MergeFlatMLP3(IPass& self) : self_{self} {}
  ...
  static std::unique_ptr<Rule> create_rule(IPass* self) {
    ...
    return Rule::create_rule(
        result, [=](onnxruntime::Graph* graph, binder_t& binder) -> bool {
          ...
          auto silu_node_quant = binder
              ["/model/layers.31/mlp/Slice_output_0_QuantizeLinear_Output"];
          ...
          std::vector<std::string> input_types_slilu_mm{
              "uint16", "int4", "int64", "int32", "int32", "int32"};
          ...
          matmulsilu_builder.set_op_type("QConv2MatMul", "com.xilinx");
          matmulsilu_builder.clone_data_type(*silu_node.node);
          // matmulsilu_builder.clone_attrs(*silu_node.node);
          matmulsilu_builder.clone_attrs(*silu_node_quant.node);
          // matmulsilu_builder.set_anchor_point1(*silu_node.node);
          matmulsilu_builder.set_anchor_point1(*silu_node_quant.node);
          ...
          matmulsilu_builder.add("in_dtypes", input_types_slilu_mm);
...
DEFINE_VAIP_PASS(MergeFlatMLP3, vaip_pass_dd_merge_flatmlp3)
