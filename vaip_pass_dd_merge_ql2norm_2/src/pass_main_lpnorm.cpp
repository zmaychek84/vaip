...
struct MergeQLPnorm_2 {
  MergeQLPnorm_2(IPass& self) : self_{self} {}
  std::unique_ptr<Rule> create_rule(IPass* self) {

    auto q1 = vaip::pattern_zoo::get_pattern("m_lpnorm_2");
    CHECK(q1 != nullptr) << "Pattern returned is null";

    return Rule::create_rule(
        q1, [=](onnxruntime::Graph* graph, binder_t& binder) -> bool {
          ...
          auto q1_node = binder["LpNormalization_0"];
          auto q1_s_node = binder["a_s"];
          auto q1_z_node = binder["a_z"];
          auto q1_sc =
              node_arg_get_const_data_as_float(*graph, *q1_s_node.node_arg);
          auto q1_zp = vaip::dd::get_zp_from_node(*graph, *q1_z_node.node_arg);

          std::vector<int32_t> qdq_coeffs(16, 0);

          // 0,1,2, - input_dequant -enable
          // 3,4,5, - output_quant -enable
          qdq_coeffs[0] = a_zp;
          qdq_coeffs[1] = vaip::dd::qmatmulcalc::float_to_bfloat16(a_sc);
          qdq_coeffs[2] = 1; // in_dequant_enable
          qdq_coeffs[3] = q1_zp;
          qdq_coeffs[4] =
              vaip::dd::qmatmulcalc::float_to_bfloat16(1.0f / q1_sc);
          qdq_coeffs[5] = 0; // out_quant_enable
          auto node_name = node_arg_get_name(*q1_node.node_arg);
          ...
          NodeBuilder(*graph, self_)
              ...
              .clone_data_type(*q1_node.node_arg)
              .clone_shape(*q1_node.node_arg)
              .set_anchor_point1(*q1_node.node_arg)
...
DEFINE_VAIP_PASS(MergeQLPnorm_2, vaip_pass_dd_merge_qlpnorm_2)
