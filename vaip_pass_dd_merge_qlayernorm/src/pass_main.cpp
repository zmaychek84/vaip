...
struct MergeQLayerNorm {
  MergeQLayerNorm(IPass& self) : self_{self} {}
  ...
  static std::unique_ptr<Rule> create_rule(IPass* self) {
    auto q = vaip::pattern_zoo::get_pattern("m_qlayernorm_0");
    ...
    return Rule::create_rule(
        q, [=](onnxruntime::Graph* graph, binder_t& binder) -> bool {
          ...
          auto a_shape_ptr = node_arg_get_shape_i64(*a_node.node_arg);
          std::vector<int64_t> a_shape;
          for (int i = 0; i < a_shape_ptr->size(); i++) {
            a_shape.push_back(a_shape_ptr->at(i));
          }
          ...
          NodeBuilder(*graph, *self)
              ...
              .add("input_shape", a_shape)
              .add("act_zp", (int64_t)act_zp) // attribute for psw
              .build();
          return true;
        });
  }
...
DEFINE_VAIP_PASS(MergeQLayerNorm, vaip_pass_dd_merge_qlayernorm)